...
def cutDownStn(text, maxLen):
    while(len(text) > maxLen):
        sentence_list = text.split('，')
        len_sentence = []
        for i in range(len(sentence_list)):
            if '。' not in sentence_list[i]:
                len_sentence.append(len(sentence_list[i]))
            else:
                len_sentence.append(1)
...
def getWiki(term): #查詢維基百科介紹
    def getItro(stn_len):
        ...
        text = wikipedia.summary(term, sentences=stn_len)     #抓取stn_len句介紹
        ...
        for item in article_list:
            ...
            n +=1
        return text
    text = getItro(1)       #抓取一句介紹
    #若段落過長，移除最長的句子
    text = cutDownStn(text, 81)
    return text #回傳地點介紹
...
def findLocationName(Coordinate): #依GPS座標取得地名與介紹(使用geopy)
    ...
    for number in range(10):  #移除郵遞區號
        if str(number) in loc_lists[-2]:
    ...
    Introduction = getWiki(loc_lists[1])        #呼叫 getWiki 取得"城市"介紹
    if(len(Introduction) < 27): #城市介紹太短
        Introduction = getWiki(loc_lists[0]) + Introduction #呼叫 getWiki 取得"國家"介紹
        Introduction = cutDownStn(Introduction, 81)
...
def mainPictureAddText(Location, Time): #繪製明信面反面(風景
    #bk_img = cv2.imread(inFile_path) #無法讀取中文路徑
    bk_img = cv2.imdecode(np.fromfile(inFile_path,dtype=np.uint8),-1) #可處理中文路徑
    try:
        (x,y,z) = bk_img.shape          #取得相片的長與寬(x,y)，z在這裡用不到
    except:
        (x,y) = bk_img.shape          #取得相片的長與寬(x,y)，z在這裡用不到
    ...
    #寫入並回傳長寬
    #cv2.imwrite(str(Path(sys.argv[0]).parent.joinpath('postcard_' + now + '_back.jpg')),bk_img) #無法使用中文路徑
    cv2.imencode('.jpg', bk_img)[1].tofile(str(Path(sys.argv[0]).parent.joinpath('postcard_' + now + '_back.jpg'))) #可處理中文路徑
...
def makePostcard(x, y, introduction, textTag, textUsr):             #繪製明信片正面(郵務
    ...
    cv2.imencode('.jpg', postcard)[1].tofile(str(Path(sys.argv[0]).parent.joinpath('postcard_' + now + '_front.jpg'))) #可處理中文路徑
    
    #繪製文字(國家介紹、個人敘述、Tag)
    #img = cv2.imread(str(Path(sys.argv[0]).parent.joinpath('postcard_' + now + '_front.jpg'))) #讀檔，無法讀中文
    img = cv2.imdecode(np.fromfile(str(Path(sys.argv[0]).parent.joinpath('postcard_' + now + '_front.jpg')),dtype=np.uint8),-1) #可處理中文路徑
    ...
    cv2.imencode('.jpg', img)[1].tofile(str(Path(sys.argv[0]).parent.joinpath('postcard_' + now + '_front.jpg')))
...
def main(): #主函數(用戶介面)
    ...
    def process(): #建立圖片處理函數，供按鈕元件呼叫
        ...
        #out_file = photoProcess(path, textTag, textUsr) #照片分析並製作明信片Debug用
        try:
